...
import be.kdg.programming.integrationproject.dao.TurnDao;
...
public class GamePresenter {
    ...
    public GamePresenter(Game game, GameView view, MainMenuView mainMenuView) {
        ...
        this.moveDao = new MoveDao(this.conn);
        this.turnDao = new TurnDao(this.conn);
    ...
    private void addEventHandlers() {
        this.view.getBtnPass().setOnAction(e -> {
            if (!this.game.getLeatherPatchQueue().isEmpty()) {
                this.view.showWarningBanner("Place your leather patch first.");
                return;
            }
            this.game.pass();
            this.registerTurn(0, 0);
    ...
    private void registerTurn(int patchId, int rotation){
        try {
            // 1. Create and Insert a Turn to get a TurnID
            Turn turn = new Turn();
            turn.setGameId(game.getGameId());
            turn.setTurnStartTime(new java.sql.Time(System.currentTimeMillis()));
            turnDao.insert(turn);

            // 2. Create the Move linked to that TurnID
            Move move = new Move(
                    0, // MoveID is generated by DB
                    turn.getTurnId(),
                    patchId,
                    turn.getTurnStartTime(),
                    new java.sql.Time(System.currentTimeMillis()),
                    0, 0, // placeholders for special patches/spaces
    ...
    public void showWarningBanner(String message) {
        this.view.showWarningBanner(message);
    }
}